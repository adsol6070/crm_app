...
type DocumentChecklistNavigationProp = StackNavigationProp<
  RootStackParamList,
  "DocumentChecklist"
>;
type DocumentChecklistRouteProp = RouteProp<
  RootStackParamList,
  "DocumentChecklist"
>;

const DocumentChecklist = () => {
  const route = useRoute<DocumentChecklistRouteProp>();
  const navigation = useNavigation<DocumentChecklistNavigationProp>();
  ...
  const getChecklist = async (leadId: string) => {
    ...
    try {
      ...
      const checklist: any = await leadService.getChecklistByVisaType(
        visaCategory
      );
      const documentList = checklist.checklists.checklist;

      const uploadedDocsResponse: any = await leadService.getUploadedDocuments(
        leadId
      );
      const uploadedDocNames = uploadedDocsResponse.documents.map(
        (doc: any) => doc.name
      );
