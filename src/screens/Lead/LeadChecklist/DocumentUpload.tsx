...
type DocumentUploadNavigationProp = StackNavigationProp<
  RootStackParamList,
  "DocumentUpload"
>;
type DocumentUploadRouteProp = RouteProp<RootStackParamList, "DocumentUpload">;
...
const DocumentUpload = () => {
  ...
  const getSingleDocument = async () => {
    ...
    try {
      ...
      const uploadedDocument = response.documents.find(
        (doc: any) => doc.name === documentName
      );
      if (uploadedDocument) {
        setUploadedDocument(uploadedDocument);
        setDocumentStatus(true);
        const urlResponse: any = await leadService.getSingleDocumentUrl(
          leadId,
  ...
  const updateSingleDocumnet = async () => {
    ...
    try {
      ...
      await leadService.updateSingleDocument(
        leadId,
        uploadedDocument.filename,
        formData
      );
  ...
  const handleShareClick = async () => {
    if (typeof documentUrl === "string" && documentUrl.startsWith("http")) {
      try {
        ...
        const fileName =
          documentName + (fileExtension ? `.${fileExtension}` : "");
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              await leadService.deleteSingleDocument(
                leadId,
                uploadedDocument.filename
              );
              getSingleDocument();
