...
const LeadActions = () => {
    ...
    const getUsers = async () => {
        try {
            ...
            const filteredUsers = response.users.filter((user: any) => user.role !== 'superAdmin');
            ...
            <View style={styles.actionContainer}>
                ...
                {hasPermission(permissions, 'Leads', 'Checklist') &&
                    <ActionButton
                        ...
                        iconColor="black"
                        onPress={() => navigation.navigate("DocumentChecklist", { leadId, visaType })}
                    />}
...
const AssignModal = ({ visible, onClose, onAssign, assignees, selectedAssignees, toggleAssignee }: any) => (
    <Modal
        visible={visible}
        transparent={true}
        animationType="slide"
            ...
            <View style={styles.modalContent}>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        ...
                        >
                            <Text style={styles.checkboxLabel}>{item.firstname} {item.lastname}</Text>
                            <MaterialIcons
                                name={selectedAssignees.includes(item.id) ? 'check-box' : 'check-box-outline-blank'}
                                size={24}
