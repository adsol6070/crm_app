...
type LeadHistoryNavigationProp = StackNavigationProp<
  RootStackParamList,
  "LeadHistory"
>;
type LeadHistoryRouteProp = RouteProp<RootStackParamList, "LeadHistory">;

const LeadHistory = () => {
  const route = useRoute<LeadHistoryRouteProp>();
  const navigation = useNavigation<LeadHistoryNavigationProp>();
  ...
  const renderItem = ({ item, index }: any) => {
    ...
    switch (item.action) {
      case "Created":
        icon = (
          <MaterialIcons name="create" size={20} color={theme.COLORS.black} />
        );
        break;
      case "Status Updated":
        icon = (
          <MaterialIcons name="update" size={20} color={theme.COLORS.black} />
        );
        break;
      case "Updated":
        icon = (
      ...
      default:
        icon = (
          <MaterialIcons name="info" size={20} color={theme.COLORS.black} />
        );
    }
      ...
      <View style={styles.itemContainer}>
        ...
        <View style={styles.itemContent}>
          ...
          <View style={styles.detailsContainer}>
            {(item.details.previousStatus == null
              ? "new"
              : item.details.previousStatus) &&
              item.details.upcomingStatus && (
                  ...
                  <View style={styles.progressLine}>
                    <View
                      style={[
                        ...
                        {
                          backgroundColor: getStatusColor(
                            item.details.upcomingStatus == null
                              ? "new"
                              : item.details.upcomingStatus
                          ),
                        },
            ...
            {item.details.statusUpdatedBy && (
              <Text style={styles.details}>
                Status updated by {item.details.statusUpdatedBy.firstname}{" "}
                {item.details.statusUpdatedBy.lastname}
              </Text>
            )}
            {item.details.updatedBy && (
              <Text style={styles.details}>
                Updated by {item.details.updatedBy.firstname}{" "}
                {item.details.updatedBy.lastname}
              </Text>
            )}
            {item.details.assignedAgents &&
              item.details.assignedAgents.length > 0 && (
                <Text style={styles.details}>
                  Assigned to{" "}
                  {item.details.assignedAgents
  ...
  return (
    <SafeAreaView style={styles.container}>
      {renderHeader()}
      {loading ? (
        <skeletonLoader.ListSkeletonLoader itemCount={4} itemHeight={100} />
      ) : (
        <FlatList
